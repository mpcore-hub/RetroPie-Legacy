...
function sources_fruitbox() {
    ...
    gitPullOrClone "$md_build/fruitbox" "https://github.com/mpcore-hub/fruitbox.git"
...
function build_fruitbox() {
    ...
    cmake .. -DSHARED=off
    make -j4 && make install
    export PKG_CONFIG_PATH=/opt/retropie/emulators/fruitbox/build/allegro5/build/lib/pkgconfig
    ldconfig
    cd ../..
...
function dbscan_fruitbox() {
    if [[ -e "$home/RetroPie/roms/jukebox/fruitbox.db" ]]; then
        rm -rf "$home/RetroPie/roms/jukebox/fruitbox.db"
    fi
